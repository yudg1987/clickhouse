            ...
            for (List<ColumnVo> list : values) {
                ...
                for (int i = 0, size = list.size(); i < size; i++) {
                    ...
                    if (StringUtils.isBlank(columnVo.getValue())) {
                        if (dataType == Types.INTEGER || dataType == Types.FLOAT
                                || dataType == Types.BIGINT || dataType == Types.BIT
                                || dataType == Types.DOUBLE || dataType == Types.DOUBLE) {// Int64
                        ...
                        if (dataType == Types.DATE) {// DATE类型
                            valueSQLTemp.append("toDateTime('" + myDateFmt.format(columnVo.getValue()) + "')");
                        } else if (dataType == Types.TIME || dataType == Types.TIMESTAMP) {// timestamp datetime
                            valueSQLTemp.append("toDateTime('" + myFmt.format(columnVo.getValue()) + "')");
                        } else if (dataType == Types.INTEGER || dataType == Types.FLOAT
                                || dataType == Types.BIGINT || dataType == Types.BIT
                                || dataType == Types.DOUBLE || dataType == Types.DOUBLE) {// Int64
                            valueSQLTemp.append(columnVo.getValue());
